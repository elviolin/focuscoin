...
begin
    ...
    if v_row.last_added_at is not null then
        v_remain := greatest(0, 60 - floor(extract(epoch from (now() - v_row.last_added_at)))::integer);
    end if;
    return json_build_object('ok', true, 'cups', v_row.cups, 'claimed', v_row.claimed, 'cooldown_remain_sec', v_remain);
end;
$$;

-- 5) RPC: 물 등록 (서버에서 1분 쿨다운 강제 — 클라이언트 조작 불가)
...
begin
    ...
    if v_row.last_added_at is not null
       and now() - v_row.last_added_at < interval '1 minute' then
        v_remain := greatest(0, 60 - floor(extract(epoch from (now() - v_row.last_added_at)))::integer);
    ...
    return json_build_object('ok', true, 'cups', v_row.cups, 'claimed', v_row.claimed, 'cooldown_remain_sec', 60);
end;
$$;
